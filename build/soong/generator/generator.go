...
func init() {
	android.RegisterModuleType("ion_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = ionExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = ionExpandVariables(ctx, dep_file)
	...
	cmd := ionExpandVariables(ctx, String(g.properties.Cmd))
